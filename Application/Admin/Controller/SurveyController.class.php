...
class SurveyController extends AdminController
{
    ...
    public function config()
    {
        ...
        $admin_config->title('社情民意基本设置')
            ->keyBool('NEED_VERIFY', '创建社情民意是否需要审核','默认无需审核')
            ->buttonSubmit('', '保存')->data($data);
        $admin_config->display();
    }
    public function survey($page = 1, $r = 10)
    {
        //读取列表
        $map = array('status' => 1);
        $model = $this->pollsModel;
        ...
        $builder->title('内容管理')
            ...
            ->keyId()->keyLink('title', '标题', 'Polls/Index/detail?id=###')->keyUid()->keyCreateTime()->keyStatus()->keyMap('is_recommend', '是否推荐', array(0 => '否', 1 => '是'))
            ->data($list)
