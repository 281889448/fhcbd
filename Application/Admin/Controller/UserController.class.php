...
class UserController extends AdminController
{
	...
	public function expandinfo_details($uid=0)
    {
     ...
     //   $map_profile['profile_group_id'] = array('in', $field_group_ids);
	      $map_profile['profile_group_id'] = array('in', array(WEIYUAN));
     ...
     *
     */
    public function team_details($uid=0){
	    $map['uid'] = $uid;
	    $map['status'] = array('egt', 0);
	    $member = M('Member')->where($map)->find();
	    $member['id'] = $member['uid'];
	    ...
	    $this->assign('profile_group_id',TEAM);
	    $this->assign('member',$member);
	    $this->display();
    }
	 ...
	 *
	 */
	public function unit_details($uid=0){
		$map['uid'] = $uid;
		$map['status'] = array('egt', 0);
		$member = M('Member')->where($map)->find();
		$member['id'] = $member['uid'];
		...
		$field_group_ids = array_column($field_group, 'id');
	//	$map_profile['profile_group_id'] = array('in', $field_group_ids);
		$map_profile['profile_group_id'] = array('in', array(UNIT));
		$fields_list = D('field_setting')->where($map_profile)->getField('id,field_name,form_type');
		$fields_list = array_combine(array_column($fields_list, 'field_name'), $fields_list);
		$map_field['uid'] = $member['uid'];
		foreach ($fields_list as $key => $val) {
		...
		$this->assign('profile_group_id',UNIT);
		$this->assign('member',$member);
		$this->display();
	}
    ...
    {	
		...
		//获取委员所在的组号
		$groupid = M('AuthGroup')->where("title='委员'")->getField('id');
...
/*---------------------------------start-------------------------------------------------*/
     ...
     *  create: 2016/9/26
     */
    public function setMake($Model=CONTROLLER_NAME){
        $ids    =   I('request.ids');
        $is_make =   I('request.is_make');
        if(empty($ids)){
            $this->error('请选择要操作的数据');
        }
        ...
        switch ($is_make){
            ...
            case 0  :
                $this->domake($Model, $map, array('success'=>'设置成功','error'=>'设置失败'));
                break;
            case 1  :
                $this->unmake($Model, $map, array('success'=>'设置成功','error'=>'设置失败'));
    ...
    protected function domake ( $model , $where = array() , $msg = array( 'success'=>'状态禁用成功！', 'error'=>'状态禁用失败！')){
        $data    =  array('is_make' => 0);
        $this->editRow( $model , $data, $where, $msg);
    }

    protected function unmake (  $model , $where = array() , $msg = array( 'success'=>'状态恢复成功！', 'error'=>'状态恢复失败！')){
        $data    =  array('is_make' => 1);
        $this->editRow(   $model , $data, $where, $msg);
    }
/*-=---------------------------end-----------------------------------*/
}
