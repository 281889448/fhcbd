...
class GroupController extends AdminController
{
     ...
     *
     */
    public function users($page = 1, $group_id = null, $r = 20, $title = '', $content = ''){
	    //读取群组基本信息
	    if ($group_id) {
	   ...
	   // $list = D('User/Weiyuan')->getWeiyuans();
	    $mu = D('User/User');
	    $mu->setModel(WEIYUAN);
	    $list = $mu->getUsers($proposal_content['uid']);
	    
	    //显示页面
	    $builder = new AdminListBuilder();
	    $builder->title('用户管理' . $groupTitle)
	            ->setStatusUrl(U('Group/setPostStatus'))->buttonEnable()->buttonDisable()->buttonDelete()
	            ->keyId()->keyText('真实姓名', '姓名', 'Group/reply?post_id=###')
	            ->keyText('reg_time','注册时间')->keyUpdateTime()->keyTime('last_reply_time', '最后回复时间')->keyText('top', '是否置顶')->keyStatus()->keyDoActionEdit('editPost?id=###')
