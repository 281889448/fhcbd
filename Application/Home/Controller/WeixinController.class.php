...
class WeixinController extends BaseController {
	...
	function __construct()
	{
		...
		if($this->openid){
			...
			$this->uid=$userdb['id'];
            $userdb['uid'] = $userdb['id'];
			if(empty($userdb)){

				$this->tobind=true;
			}
           session('user_auth',$userdb);
            session('user_auth_sign', data_auth_sign($userdb));
	...
	public function savebind(){
		...
		$userdb=D('Ucenter_member')->where(array('username'=>I('post.mobile'),'password'=>$password))->find();
		if($userdb){
