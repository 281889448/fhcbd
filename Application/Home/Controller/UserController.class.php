        ...
        if (IS_POST) { //登录验证
            ...
            if (0 < $uid) { //UC登录成功
                ...
                if ($Member->login($uid, $remember == 'on')) { //登录用户
                   ...
                   $rediect_url = str_replace('\\','/',unserialize(I('get.redirect_url')));
                    $rediect_url = $rediect_url ? $rediect_url : get_nav_url(C('AFTER_LOGIN_JUMP_URL'));
                        $this->success($html,$rediect_url);
