...
//同步到微博
    ...
    public function save($id){
        ...
        $result = D("Event")->where("id={$id}")->save($_POST);
    ...
    public function inset($type='',$title ='', $sTime = '', $eTime = '', $address = '',$people='',$content='')
    {
        ...
        $data['qdstime']=implode(',',$_POST['qdstime']);
        $data['qdetime']=implode(',',$_POST['qdetime']);
        $result = D('Event')->add($data);
        if ($result == null) {
            $this->error("添加失败","{:U('Event/Index/index')}");
        }else{
            $this->success("添加成功");
        }
