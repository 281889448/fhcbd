 ...
 */
    ...
    class CheckinModuleAddon extends Addon{
        ...
        $data = S('checkmodule_info_'); //model('Cache')->get('check_info_' . $uid . '_' . date('Ymd'));
		$data['module'] = $module;
        if (!$data) {
            $map['uid'] = $uid;
            $map['ctime'] = array('gt', strtotime(date('Ymd')));
            $res = D('Checkmodule_info')->where($map)->find();
            //是否签到
            $data['ischeck'] = $res ? true : false;

            $checkinfo = D('Checkmodule_info')->where('uid=' . $uid)->order('ctime desc')->limit(1)->find();
            if ($checkinfo) {
        ...
        $list = D('Checkmodule_info')->where('uid=' . $uid)->order('ctime desc')->count();
